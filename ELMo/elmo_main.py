...
class BiLSTMAttention(object):
    ...
    def __init__(self, config):
        ...
        outputs = tf.split(self.embeddedWords, 2, 2)#将embeddedWords在第二维度上分成两份。
        ...
        with tf.name_scope("Attention"):
            ...
            outputSize = config.model.hiddenSizes[-1]#最后一层LSTM的维度（神经元个数）
    ...
    def _attention(self, H):
        ...
        r = tf.matmul(tf.transpose(H, [0, 2, 1]), tf.reshape(self.alpha, [-1, config.sequenceLength, 1]))#shape(batch_size,hidden_size,1)
...
with tf.Graph().as_default():
    session_conf = tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)#配置Session参数，allow_soft_placement=True，允许tf自动选择一个存在并且可用的设备来运行操作。
    #log_device_placement=True  在终端打印出各项操作是在哪个设备上运行的
    session_conf.gpu_options.allow_growth = True#allow_growth设置为True时，分配器将不会指定所有的GPU内存，而是根据需求增长
